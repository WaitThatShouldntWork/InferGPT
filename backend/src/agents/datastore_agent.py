...
async def generate_query(
    ...
    await publish_log_info_async(LogPrefix.USER, f"Cypher generated by the LLM: {llm_query}", __name__)
    if json_query["query"] == "None":
        return "No database query"
    db_response = execute_query(json_query["query"])
    await publish_log_info_async(LogPrefix.USER, f"Database response: {db_response}", __name__)
