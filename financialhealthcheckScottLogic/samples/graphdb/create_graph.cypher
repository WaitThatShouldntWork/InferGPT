...
LOAD CSV WITH HEADERS FROM "http://localhost:5500/samples/graphdb/knowledge.csv" AS row
WITH row WHERE row.knowledgeId IS NOT NULL
MERGE (:Knowledge {knowledgeId: toInteger(row.knowledgeId), name: row.name});

LOAD CSV WITH HEADERS FROM "http://localhost:5500/samples/graphdb/profiles_knowledge.csv" AS row
...
LOAD CSV WITH HEADERS FROM "http://localhost:5500/samples/graphdb/questions.csv" AS row
...
LOAD CSV WITH HEADERS FROM "http://localhost:5500/samples/graphdb/questions_knowledge.csv" AS row
